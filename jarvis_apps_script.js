...
function doPost(e) {
  ...
  try {
    ...
    else if (body.action === 'sendSelectedMail') result = sendSelectedMail(body.emails, body.subject, body.body);
...
function proxyChat(messages, systemPrompt) {
  if (!ANTHROPIC_KEY || ANTHROPIC_KEY === 'YOUR_ANTHROPIC_API_KEY_HERE') {
    return { success: false, error: 'ANTHROPIC_KEY가 설정되지 않았습니다. Apps Script 코드 상단에 API 키를 입력해주세요.' };
  }
  var payload = {
  ...
  if (code !== 200 || data.error) {
    return { success: false, error: (data.error && data.error.message) || 'API 오류 코드: ' + code };
  }
...
function getContacts(category, industry) {
  ...
  ['customer', 'partner'].forEach(function(type) {
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      out.push({
        rowIndex : i + 1, type: tp,
        company  : String(row[0] || ''), contact : String(row[1] || ''),
        email    : String(row[2] || ''), phone   : String(row[3] || ''),
        industry : ind, category: cat,
...
function saveInquiry(data) {
  ...
  sheet.appendRow([new Date(), data.name || '미입력', data.email || '',
...
function sendBulkMail(category, subject, body, industry) {
  ...
  return { success: true, sentCount: sent.length, failedCount: failed.length,
...
function logMail(data) {
  ...
  if (!sheet) return;
  sheet.appendRow([new Date(), data.to||'', data.email||'',
...
// 선택된 수신자에게만 메일 발송
// ═════════════════════════════════════════════
function sendSelectedMail(emails, subject, body) {
  if (!emails || emails.length === 0) return { success: false, error: '수신자가 없습니다.' };
  if (!subject || !body) return { success: false, error: '제목과 본문은 필수입니다.' };
  ...
  emails.forEach(function(c) {
    ...
    try {
      ...
      // 발송 이력 기록
      logMail({
        to      : (c.contact || '') + '/' + (c.company || ''),
        email   : c.email,
        subject : subject,
        category: c.category || '선택발송',
        result  : '성공',
  ...
  return {
    ...
    message     : sent.length + '명 발송 완료' + (failed.length > 0 ? ' (' + failed.length + '명 실패)' : '')
  };
}
